/**********************************************************************
* Project           : MS62
*            		: Step 3A-Single exposure models for Diabetes
* Author            : CK
...
*STANDARDIZE VARS;
 ...
 DATA renameZscore;
  SET foranalysis;
	...
	TPOPDENSL2_ZSCR = BECPOPDENSL2;
	TPOPDENSL1AD_ZSCR = BECPOPDENSL1AD;
...
proc standard data= renameZscore mean=0 std=1
              out=zscores;
var BECADINTDENSL2_ZSCR BECMEDNDVINL2_ZSCR BECPOPDENSL2_ZSCR BECPOPDENSL1AD_ZSCR TPOPDENSL2_ZSCR TPOPDENSL1AD_ZSCR CNSSE3_L2_ZSCR
...
model  diabetes= &var svyage_10 svymale svyedu iso2 /dist=bin link=logit solution or(label);
...
proc glimmix data=zscores noclprint empirical=mbn method=LAPLACE order=data;
class salid1 salid2 svymale svyedu (ref='1:Less than primary') iso2(ref='BR') ;
model  diabetes= &var svyage_10 svymale svyedu CNSSE3_L2_ZSCR BECPCTURBANL1AD_ZSCR iso2 /dist=bin link=logit solution or(label);
nloptions gconv=0.0000000001;
RANDOM INTERCEPT / TYPE=UN SUBJECT=salid1;
RANDOM INTERCEPT / TYPE=UN SUBJECT=salid2(salid1);
...
model  diabetes = BECPTCHDENSL1AD_ZSCR svyage_10 svymale svyedu iso2 /dist=bin link=logit solution or(label);
nloptions gconv=0.0000000001;
RANDOM INTERCEPT / TYPE=UN SUBJECT=salid1;
RANDOM INTERCEPT / TYPE=UN SUBJECT=salid2(salid1);
...
proc glimmix data=zscores noclprint empirical=mbn method=LAPLACE order=data;
class salid1 salid2 svymale iso2(ref='BR') svyedu (ref='1:Less than primary');
model  diabetes= BECPTCHDENSL1AD_ZSCR svyage_10 svymale svyedu CNSSE3_L2_ZSCR BECPCTURBANL1AD_ZSCR iso2 /dist=bin link=logit solution or(label);
nloptions gconv=0.0000000001;
RANDOM INTERCEPT / TYPE=UN SUBJECT=salid1;
RANDOM INTERCEPT / TYPE=UN SUBJECT=salid2(salid1);
...
proc glimmix data=zscores noclprint empirical=mbn method=LAPLACE order=data;
class salid1 salid2 svymale iso2(ref='BR') svyedu (ref='1:Less than primary');
model  diabetes = BECPTCHDENSL1AD_ZSCR svyage_10 svymale svyedu BECPCTURBANL1AD_ZSCR iso2 /dist=bin link=logit solution or(label);
nloptions gconv=0.0000000001;
RANDOM INTERCEPT / TYPE=UN SUBJECT=salid1;
RANDOM INTERCEPT / TYPE=UN SUBJECT=salid2(salid1);
...
else if index (label,"CNSSE3") then order = 13;
run;
...
**COMBINE THREE MODELS FOR FRAGMENTATION;
...
ODS listing close;
ODS tagsets.ExcelXP file = "......\output\Diabetes Models (1) and (2)  01-19.xml" 
style=minimal
...
column label   ("Model 1 Diabetes = Age+sex+EDU+&label" ("Model 1" Estimatemodel1 StdErrmodel1 Tvaluemodel1 Probtmodel1))
               ("Model 2 Diabetes = Age+sex+EDU+PCT Urban+AnaOScore+country+&label"("Model 2" Estimatemodel2 StdErrmodel2 Tvaluemodel2 Probtmodel2));
...
%table (BECPOPDENSL2_ZSCR, Population Density L2); 
...
column label   ("Model 1 Diabetes = Age+sex+EDU+&label"                             ("Model 1" Estimatemodel1 StdErrmodel1 Tvaluemodel1 Probtmodel1))
               ("Model 2 Diabetes = Age+sex+EDU+PCT Urban+AnaOScore+country+&label" ("Model 2" Estimatemodel2 StdErrmodel2 Tvaluemodel2 Probtmodel2))
			   ("Model 3 Diabetes = Age+sex+EDU+AnaOScore+country+&label"           ("Model 3" Estimatemodel3 StdErrmodel3 Tvaluemodel3 Probtmodel3))
               ("Model 4 Diabetes = Age+sex+EDU+PCT Urban+&label"                   ("Model 4" Estimatemodel4 StdErrmodel4 Tvaluemodel4 Probtmodel4));
